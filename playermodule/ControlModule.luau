...
local realTouchDPad = require(script:WaitForChild("TouchDPad"))
local TouchDPad2 = require(script:WaitForChild("TouchDPad2"))
...
-- Returns module (possibly nil) and success code to differentiate returning nil due to error vs Scriptable
local modeMap = { m1 = realTouchDPad, m2 = TouchDPad2 }
local modeAttributeKey = "MobileDPad"
function ControlModule:SelectTouchModule()
    ...
    if DevMovementMode == Enum.DevTouchMovementMode.UserChoice then
        --@edit: D-Pad re-implementation
        local value = Players.LocalPlayer:FindFirstChild("KitConfiguration")
        if value and modeMap[value:GetAttribute(modeAttributeKey)] then
            return modeMap[value:GetAttribute(modeAttributeKey)], true
        end
    ...
    if not controlModule then
        ...
        if self.activeController ~= self.controllers[controlModule] then
            ...
            if self.touchControlFrame and (self.activeControlModule == TouchThumbpad
                ...
                or self.activeControlModule == TouchDPad2) then
